...
class Car(object):
    ...
    def generateTraj(self):
        for _ in range(self.n_poses):
            T = np.array([[np.cos(self.theta), -np.sin(self.theta), 0, self.x],
            ...
            if self.apply_noise:
                ...
                T = local_perturb.compose(pose_gtsam).matrix()
    ...
    def generatePoints(self):
        ...
        if self.apply_noise:
            local_pts += np.random.normal(loc=0, scale=self.point_noise, size=local_pts.shape)
